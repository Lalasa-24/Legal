...
<script>
    ...
    const departments = Object.keys(data).filter(dept => dept !== 'overall');
    const agreementTypes = Array.from(new Set(departments.flatMap(dept => Object.keys(data[dept]))));
    const overallData = data.overall;

    // Preparing data for the chart
//    const departmentDatasets = departments.map(department => ({
//        label: `${department} Department Agreements`,
//        data: agreementTypes.map(type => data[department][type] || 0),
...
//        data: agreementTypes.map(type => overallData[type] || 0),
...
//
//    // Combining datasets
//    const datasets = [...departmentDatasets, overallDataset];
// Instead of mapping over all departments, choose one department (e.g., the first one)
const selectedDepartment = departments[0];  // Choose the first department, or select any other specific one

const departmentDataset = {
    label: `${selectedDepartment} Department Agreements`,
    data: agreementTypes.map(type => data[selectedDepartment][type] || 0),
    backgroundColor: 'rgba(0,57,116,0.6)',
    borderColor: 'rgba(0,57,116,1)',
    borderWidth: 1
};
...
// Combine only one department's dataset and the overall dataset
const datasets = [departmentDataset, overallDataset];

    const ctx = document.getElementById('agreementChart').getContext('2d');
    ctx.canvas.width = 750;  // Set the width in pixels
    ctx.canvas.height = 350; // Set the height in pixels

    new Chart(ctx, {
        type: 'bar',
        data: {
            labels: agreementTypes,
            datasets: datasets
        },
        options: {
            responsive: false,
