...
@WebServlet("/FilteredCommencementDateServlet")
public class FilteredCommencementDateServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (executionDate != null && !executionDate.isEmpty() && 
            ...
            try (Connection conn = DriverManager.getConnection(DBParameters.URL, DBParameters.USER, DBParameters.PASSWORD);
                 CallableStatement stmt = conn.prepareCall("{CALL GetCommencementDateFilteredAgreements(?, ?, ?)}")) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        ...
                        responseData.put(commencementType, count);
                    }
                }
                ...
                if ("XMLHttpRequest".equals(request.getHeader("X-Requested-With"))) {
                    ...
                    request.getRequestDispatcher("FilteredCommencementDateA.jsp").forward(request, response);
                }
