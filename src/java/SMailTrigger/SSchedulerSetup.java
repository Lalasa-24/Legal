...
public class SSchedulerSetup {

    public static void main(String[] args) throws SchedulerException {
        Scheduler scheduler = StdSchedulerFactory.getDefaultScheduler();
        scheduler.start();

        List<SContract> contracts = SDatabaseUtil.getContracts();

        for (SContract contract : contracts) {
            ...
            if (expiryDate != null) {
                ...
                scheduleJob(scheduler, contract, expiryDate, "Expiry Alert", 0); // Expiry alert job
            } else {
    ...
    private static void scheduleJob(Scheduler scheduler, SContract contract, Date expiryDate, String alertType, int daysBeforeExpiry) throws SchedulerException {
        Calendar cal = Calendar.getInstance();
        cal.setTime(expiryDate);
        cal.add(Calendar.DAY_OF_MONTH, daysBeforeExpiry);

        JobDetail job = JobBuilder.newJob(SAlertJob.class)
